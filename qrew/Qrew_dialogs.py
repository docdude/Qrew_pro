...
class ClearMeasurementsDialog(QDialog):
    ...
    def __init__(self, measurement_count, parent=None):
        ...
        if measurement_count > 0:
            ...
            message_text = (
                "REW contains no existing measurements.\n\n"
...
class SettingsDialog(QDialog):
    ...
    def preview_theme(self):
        ...
        """
        # Apply to parent (MainWindow) if it exists
        if self.parent():
            self.parent().setPalette(palette)
            app.style().polish(self.parent())
            for widget in self.parent().findChildren(QWidget):
                widget.setPalette(palette)
                app.style().polish(widget)
            self.parent().update()  # Force update to apply new palette

        # Update the dialog's background
        self.parent().bg_opacity = 0.10 if use_light else 0.15
        set_background_image(self)

        # Update the background image opacity
        if hasattr(self, "bg_source"):
            self.bg_source.setOpacity(self.parent().bg_opacity)
            set_background_image(self)
        """
